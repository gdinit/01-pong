...
const sf::Time State::TimePerFrame = sf::seconds( 1.f /
		CONFIG_DESIRED_FPS_FLOAT );
...
void State::toggleDebugShowOverlay()
{
	if ( SETTINGS->inGameOverlay == true ) {
		...
		updateDebugOverlayTextIfEnabled( true );// give me stats in the
							// first frame, but
							// first make up some
							// plausible values
	}
...
void State::updateDebugOverlayTextIfEnabled( bool b )
{
	if ( SETTINGS->inGameOverlay ) {
		if ( b ) {
			...
			m_statisticsText.setString( std::to_string( n ) +
				" FPS \n" + std::to_string( mil / n ) +
				"us\n" );
		} else {
...
void State::updateDebugOverlayTextIfEnabled()
{
	...
	if ( SETTINGS->inGameOverlay && ( getStateAgeAsSeconds() >=
...
void State::printConsoleDebugIfEnabled()
{
	if ( SETTINGS->debugPrintToConsole ) {
		...
		std::to_string( m_statisticsUpdateTime.asMicroseconds() ) +
		"us\n";
	}
}
...
void State::recordObservedFPS()
{
	...
	if ( getStateAgeAsSeconds() <
		...
		while ( m_observedFPSLastN.size() >
			CONFIG_OBSERVED_FPS_CONTAINER_MAX_SIZE ) {
...
void State::dynamicallyAdjustFPSLimit()
{
	...
	if ( getStateAgeAsSeconds() <
		...
		if ( m_observedFPSLastN.size() >=
		     CONFIG_OBSERVED_FPS_ACT_THRESHOLD ) {
			// get median
			...
			if ( median < CONFIG_DESIRED_FPS_INT ) {
				...
				if ( delta > 10 ) {
					...
					{
						std::cout <<
						"\t(Delta=1)\tFPSLimit INTACT.\tMedian: "
						<< median << "\t";
					}
				}
				...
				if ( SETTINGS->debugPrintToConsoleFPS ) {
					std::cout <<
					"\t(median==desired)\tFPSLimit INTACT.\tMedian: "
					<< median << "\t";
				}
			}
		}
